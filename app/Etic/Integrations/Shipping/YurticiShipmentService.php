...
class YurticiShipmentService
{
    public function __construct(
        private YurticiClient $client,
        private ShippingCredentials $credentials,
    ) {}

    public function createFromOrder(Order $order): YurticiShipmentResult
    {
        ...
        if (filled(data_get($order->meta, 'yurtici.integration_code'))) {
            throw new RuntimeException('Bu sipariş için Yurtiçi gönderisi zaten oluşturulmuş.');
        }
        ...
        if (! $result->success) {
            throw new RuntimeException($result->message ?: 'Yurtiçi Kargo gönderisi oluşturulamadı.');
        }
